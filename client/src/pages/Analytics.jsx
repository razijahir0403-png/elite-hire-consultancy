...
const Analytics = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    resourcePerson: '',
    portalLink: '',
    status: RECRUITMENT_STATUS.VERIFIED,
    description: '',
  });
  ...
  const validateForm = () => {
    ...
    if (!formData.resourcePerson.trim()) {
      errors.resourcePerson = 'Resource person is required';
    }
  ...
  const handleUpdateStatusClick = (record) => {
    setActiveRecord(record);
    setStatusForm({
      status: record.status ?? RECRUITMENT_STATUS.VERIFIED,
      description: ''
    });
          ...
          <div className="space-y-1.5 col-span-1 sm:col-span-2">
            ...
            <div className="relative">
              <input
                ...
                placeholder="Search company, phone, domain, or ID..."
      ...
      <div className="flex flex-row items-center justify-between gap-4 bg-white px-5 py-3.5 rounded-2xl border border-slate-200 shadow-sm text-xs">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl text-[11px] font-bold uppercase tracking-wide transition-all shadow-sm"
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-[10px] uppercase font-extrabold tracking-wider text-slate-500">
                  <th onClick={() => handleSort('idnumber')} className="px-6 py-4 cursor-pointer hover:text-slate-800">
              ...
              <tbody className="divide-y divide-slate-100 bg-white text-xs">
                {records.map((r) => (
                  <tr key={r._id} className="hover:bg-slate-50/60 transition-colors">
                    <td className="px-6 py-4 font-extrabold text-brand-800">{r.idnumber}</td>
                    ...
                    <td className="px-6 py-4 text-slate-500">
                      <div className="text-[10px] text-slate-700 font-medium">{new Date(r.updatedOn).toLocaleDateString()}</div>
        ...
        <form onSubmit={handleAddEditSubmit} className="space-y-4 text-slate-800">
          ...
          <div>
            ...
            <input
              type="text"
              required
              placeholder="e.g. Java Specialist"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 bg-slate-50/70 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:border-brand-800 focus:ring-1 focus:ring-brand-800 text-xs"
              />
              {formErrors.resourcePerson && (
                <p className="text-[10px] text-red-600 mt-1">{formErrors.resourcePerson}</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <input
              type="url"
              placeholder="e.g. https://www.naukri.com/candidate-resume"
          ...
          {!activeRecord && (
            <>
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="Provide candidate background, qualification summaries, or sourcing details..."
          ...
          <div className="flex items-center space-x-3 pt-4 border-t border-slate-100">
            ...
            <button
              type="submit"
              className="flex-1 py-2.5 bg-gradient-to-r from-brand-800 to-blue-700 hover:from-brand-700 hover:to-blue-600 text-white font-bold text-xs rounded-xl shadow-glow-brand transition-all"
            >
        ...
        <form onSubmit={handleStatusSubmit} className="space-y-4 text-slate-800">
          <div>
            ...
            <select
              ...
              onChange={(e) => setStatusForm({ ...statusForm, status: Number(e.target.value) })}
              className="w-full px-3.5 py-2.5 bg-slate-50/70 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:border-brand-800 focus:ring-1 focus:ring-brand-800 text-xs animate-none"
            >
              {recruitmentFilterOptions.map((opt) => (
                <option key={opt.value} value={opt.value}>{opt.label}</option>
          ...
          <div>
            ...
            <textarea
              required
              rows={3}
              placeholder="Specify the reason or transition details for audit records..."
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-slate-400 font-medium">
                      {new Date(hist.updatedOn).toLocaleString()}
                    </span>
                  </div>
