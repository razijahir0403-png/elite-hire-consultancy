...
async function test() {
  try {
    ...
    console.log("Fetching CLIENTS with age='Today'");
    const res1 = await request('GET', '/api/clients?age=Today', null, token);
    console.log("Total clients returned:", res1.records ? res1.records.length : res1);
    if (res1.records && res1.records.length > 0) {
      const dates = res1.records.map(r => r.createdAt);
      console.log("Sample createdAt values:", dates.slice(0, 5));
    }

    console.log("Fetching RECEIVED-INFO with age='Today'");
    const res2 = await request('GET', '/api/received-info?age=Today', null, token);
    console.log("Total received-info returned:", res2.records ? res2.records.length : res2);
